...
BEGIN
  ...
  -- Daily bonus: +500 only on 2026-02-17 BRT
  v_brt_date := to_char(NOW() AT TIME ZONE 'America/Sao_Paulo', 'YYYY-MM-DD');
  IF v_brt_date = '2026-02-17' THEN
    v_daily_bonus := 500;
  ELSE
    v_daily_bonus := 0;
  END IF;
  ...
  IF v_token.daily_limit IS NOT NULL THEN
    ...
    v_remaining_daily := (v_token.daily_limit + v_daily_bonus) - v_used_daily - v_reserved_daily;
