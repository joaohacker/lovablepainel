...
BEGIN
  ...
  IF v_count >= p_max_requests THEN
    ...
    -- AUTO-BAN: Only after 25+ violations across ALL endpoints in 30 minutes
    -- This makes it virtually impossible for honest users to trigger
    ...
    IF v_fraud_count >= 25 THEN
      INSERT INTO public.banned_users (user_id, reason)
      VALUES (p_user_id, 'Auto-ban: ' || v_fraud_count || ' rate limit violations in 30min')
      ON CONFLICT DO NOTHING;

      IF p_ip IS NOT NULL AND p_ip <> 'unknown' THEN
        INSERT INTO public.banned_ips (ip_address, reason)
        VALUES (p_ip, 'Auto-ban: ' || v_fraud_count || ' rate limit violations in 30min')
        ON CONFLICT DO NOTHING;
      END IF;
